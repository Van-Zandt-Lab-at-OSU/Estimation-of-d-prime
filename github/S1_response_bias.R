...
p1 <- ggplot(data = df,aes(x = x,y = y)) + 
  stat_sum() + ggtitle("Impact of response bias: c=0") +
  xlab("True d'") + ylab("Estimated d'") + theme(legend.position = "none",
                                                 plot.title = element_text(hjust = 0.5)) +
  geom_abline(slope=0, intercept=3,col="blue") +
  geom_point(data=sub,aes(x=x,y=y),col='red') + ylim(1,4)
...
p2 <- ggplot(data = df,aes(x = x,y = y)) + 
  stat_sum() + ggtitle("Impact of response bias: c=0.5") +
  xlab("True d'") + ylab("Estimated d'") + theme(legend.position = "none",
                                                 plot.title = element_text(hjust = 0.5)) +
  geom_abline(slope=0, intercept=3,col="blue") +
  geom_point(data=sub,aes(x=x,y=y),col='red') + ylim(1,4)
...
p3 <- ggplot(data = df,aes(x = x,y = y)) + 
  stat_sum() + ggtitle("Impact of response bias: c=1") +
  xlab("True d'") + ylab("Estimated d'") + theme(legend.position = "none",
                                                 plot.title = element_text(hjust = 0.5)) +
  geom_abline(slope=0, intercept=3,col="blue") +
  geom_point(data=sub,aes(x=x,y=y),col='red') + ylim(1,4)
