...
# signal and noise trials
  ...
  p1 <- ggplot(data = df,aes(x = x,y = y)) + 
    stat_sum() + ggtitle("Impact of trial size: 20 signal + 20 noise trials") +
    xlab("True d'") + ylab("Estimated d'") + theme(legend.position = "none",
                                                   plot.title = element_text(hjust = 0.5)) +
    geom_abline(slope=1, intercept=0,col="blue") +
    geom_point(data=sub,aes(x=x,y=y),col='red') + ylim(-0.5,5.5)
  ...
  p2 <- ggplot(data = df,aes(x = x,y = y)) + 
    stat_sum() + ggtitle("Impact of trial size: 100 signal + 100 noise trials") +
    xlab("True d'") + ylab("Estimated d'") + theme(legend.position = "none",
                                                   plot.title = element_text(hjust = 0.5)) +
    geom_abline(slope=1, intercept=0,col="blue") +
    geom_point(data=sub,aes(x=x,y=y),col='red') + ylim(-0.5,5.5)
